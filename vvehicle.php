...
body, html {
    ...
    if($result->num_rows>0){
       echo "<br><br>";
        echo "<center>";
        echo "<table border='1'>";
        echo "<tr id ='r1'><th><strong>Vehicle ID</strong></th><th><strong>Modal Name</strong></th><th><strong>Color</strong></th><th><strong>Price</strong></th><th><strong>Location ID</strong></th>";
        while($row=$result->fetch_assoc())
        {
           echo"<tr><td>{$row["vid"]}</td><td>{$row["v_name"]}</td><td>{$row["color"]}</td><td>{$row["price"]}</td><td>{$row["loc_id"]}</td></tr>";
